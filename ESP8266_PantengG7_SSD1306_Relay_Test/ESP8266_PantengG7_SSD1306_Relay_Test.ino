...
void OLedDisplay(const int *pm_ug, const int *pm_cnt) {

  display.clear();

  sprintf(strBuf, "PM10: %d, %d(cf)", *(pm_ug + 2), *(pm_ug + 5));
  display.drawString(0, 2, strBuf);
  sprintf(strBuf, "PM2.5: %d, %d(cf)", *(pm_ug + 1), *(pm_ug + 4));
  display.drawString(0, 20, strBuf);
  sprintf(strBuf, "PM1.0: %d, %d(cf)", *(pm_ug), *(pm_ug + 3));
